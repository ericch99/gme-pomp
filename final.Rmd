...
gme %>% 
  pomp(times="day",t0=0,
       ...
       dmeasure=dgme,
       accumvars="H", 
       partrans=parameter_trans(log=c("N", "Beta1", "Beta2", "mu_S1S2", "mu_IR", "mu_RS1"),logit=c("rho", "eta")),
...
paste0('standard error: ', L1[2])
```
<!-- ACTUAL GLOBAL SEARCH -->

```{r grid, cache=TRUE, echo=FALSE}
runif_design(lower=c(Beta1=0.1,Beta2=0.1,mu_S1S2=0.1,mu_IR=0.1,mu_RS1=0.01,eta=0.1,rho=0.05), 
             upper=c(Beta1=5,Beta2=5,mu_S1S2=5,mu_IR=5,mu_RS1=0.1,eta=1,rho=0.3),
             nseq=Nseq) -> guesses

guesses$N = runif(length(guesses$rho), 100000, 200000) * guesses$rho
```
...
foreach(guess=iter(guesses,"row"), .export=c("Np", "Nmif"), .combine=rbind) %dopar% {
  ...
  SSIR %>% mif2(params=c(unlist(guess)),
                ...
                rw.sd=rw.sd(Beta1=0.02,Beta2=0.02,mu_S1S2=0.02, mu_IR=0.02, mu_RS1=0.02, eta=ivp(0.02))) %>% mif2(Nmif) -> mf 
...
arrange(type) -> all
pairs(~loglik+Beta1+Beta2+mu_S1S2+mu_IR+mu_RS1+eta, data=all,
