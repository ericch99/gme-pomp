...
results[results$loglik < -293,] %>% 
  filter(is.finite(loglik)) %>%
  filter(loglik>max(loglik)-20,loglik.se<2) -> final

apply(final, 2, function(x) mean(x)+2.5*sd(x))

final %>% filter(Beta < 2.5, mu_RS < 10) %>% sapply(range) -> box
box
```
...
all %>% group_by(round(mu_RS,4)) %>% ungroup() %>%
...
all %>% group_by(round(eta,4)) %>% ungroup() %>%
