...
Given our analyses above, we find it appropriate to model our data with a compartmental model, commonly used in epidemiological studies -- specifically, we use a **SIRS model**.
...
Consider that the popularity of a trend can *resurge* long after the initial event; indeed, this behavior seems to be evident in the data. Because of this, we suppose that recovered individuals (represented by the state $R$) move back into the initial susceptible state $S_1$ after some time. 
...
params <- c(Beta=0.9,mu_IR=0.5,mu_RS=0.03,rho=0.8,eta=0.8,N=4000)
...
Since the search frequency data are normalized to have maximum $100$, it is difficult to interpret the reporting rate $\rho$ and population size $N$. However, Statista tells us that there were 267 million unique Google users from the US in 2020  [[9](https://www.statista.com/topics/1001/google/)]. Under some reasonable assumptions <sup style='color:gray'> [\*] </sup>, this leads to an estimated reporting rate of $\rho = 0.8$, which we will fix for simplicity. We will also fix the population size $N=3000$ to match the simulations above.
...
Nseq <- switch(run_level, 50, 500, 5000)
...
runif_design(lower=c(Beta=0.1,mu_IR=0.1,mu_RS=0.01,eta=0.1,rho=0.8, N=4000), 
             upper=c(Beta=2,mu_IR=1,mu_RS=0.1,eta=1,rho=0.8, N=4000),
             nseq=Nseq) -> guesses
...
foreach(guess=iter(guesses,"row"), .export=c("Np", "Nmif"), .combine=rbind) %dopar% {
  library(pomp)
  library(tidyverse)
  ...
  SSIR %>% mif2(params=c(unlist(guess)),
                ...
                rw.sd=rw.sd(Beta=0.02, mu_IR=0.02, mu_RS=0.02, eta=ivp(0.02))) %>% mif2(Nmif) -> mf 
...
final_results[final_results$loglik == max(final_results$loglik),] %>%
  select(-one_of(c('rho','N', 'loglik', 'loglik.se'))) %>%
  as.matrix %>%
  as.table() %>%
  print()

final_results[final_results$loglik == max(final_results$loglik),] %>%
  select(one_of(c('loglik', 'loglik.se'))) %>%
  as.matrix %>%
  as.table() %>%
  print()
```
